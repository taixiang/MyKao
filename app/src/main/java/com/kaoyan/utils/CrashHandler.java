...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ex.printStackTrace();
        LogUtil.e("YanTuException == "+ex.getMessage());
        for(int i=0; ex.getCause()!= null && i< ex.getCause().getStackTrace().length ;i++){
            LogUtil.e("YanTuException == "+ex.getCause().getStackTrace()[i]);
        }
...
//            public void run() {
//                Looper.prepare();
//                ToastUtils.showToast(mContext,"很抱歉，程序出现异常，即将重启");
//                Looper.loop();
//            }
//        }.start();
